...
export default function UpdateForm({ config, formElements, withUpload = false }) {
  ...
  useEffect(() => {
    if (current) {
      ...
      // Handle populated objects (extract _id for Select components)
      Object.keys(newValues).forEach((key) => {
        if (newValues[key] && typeof newValues[key] === 'object' && newValues[key]._id) {
          newValues[key] = newValues[key]._id;
        }
      });
