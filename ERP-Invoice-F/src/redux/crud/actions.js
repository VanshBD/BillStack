  ...
  list:
    ...
    async (dispatch) => {
      ...
      if (data.success === true) {
        const result = {
          items: Array.isArray(data.result) ? data.result : data.result?.docs || [],
          pagination: {
            current: parseInt(data.pagination?.page || data.result?.page || options?.page || 1, 10),
            pageSize: options?.items || 10,
            total: parseInt(data.pagination?.count || data.result?.totalDocs || 0, 10),
          },
        };
        dispatch({
